...
struct landing_page: View {
    var body: some View {
        VStack {
            ...
            Spacer()
            Color.white.frame(minWidth: 200, maxWidth: .infinity, maxHeight: 200).overlay(
                VStack { // Change 'Group' to 'VStack'
                    ...
                    Button(action: {print("Tapped")}){
                        Text("Search Now!").fontWeight(.bold).font(.title).padding().background(Color(hex: 0x63A313)).cornerRadius(40).foregroundColor(.white).padding(10).overlay(RoundedRectangle(cornerRadius: 40).stroke(Color(hex: 0x63A313), lineWidth: 5)
                        )
                    }
                }
            ).ignoresSafeArea()
